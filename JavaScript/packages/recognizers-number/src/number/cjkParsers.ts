...
export class BaseCJKNumberParser extends BaseNumberParser {
    ...
    parse(extResult: ExtractResult): ParseResult | null {
        ...
        const getExtResult: ExtractResult = {
    ...
    private perParseCJK(extResult: ExtractResult): ParseResult {
        ...
        if (extResult.data.includes('Spe')) {
            ...
            if (resultText === '半額' || resultText === '半折' || resultText === '半折') {
                ...
                const matches = RegExpUtility.getMatches(this.config.speGetNumberRegex, resultText);
                let intNumber: number;

                if (matches.length === 2) {
                    const intNumberChar = matches[0].value.charAt(0);
                    ...
                    const pointNumberChar = matches[1].value.charAt(0);
                    ...
                    const intNumberChar = matches[0].value.charAt(0);
                    const pointNumberChar = matches[1].value.charAt(0);
                    const dotNumberChar = matches[3].value.charAt(0);

                    const pointNumber = this.config.zeroToNineMap.get(pointNumberChar)! * 0.1;
                    const dotNumber = this.config.zeroToNineMap.get(dotNumberChar)! * 0.01;
            ...
            const doubleMatch = RegExpUtility.getMatches(this.config.percentageRegex, resultText).pop()!;
            const doubleText = doubleMatch.value;
            ...
            const doubleMatch = RegExpUtility.getMatches(this.config.percentageRegex, resultText).pop()!;
            const doubleText = this.replaceUnit(doubleMatch.value);

            const splitResult = RegExpUtility.split(this.config.pointRegex, doubleText);
            if (splitResult[0] === '') {
        ...
        const perMatch = RegExpUtility.getMatches(this.config.percentageNumRegex!, resultText);
        if (perMatch.length > 0) {
            const splitResult = perMatch[0].value;
            const splitResultList = RegExpUtility.split(this.config.fracSplitRegex, splitResult);
            const demoValue = this.isDigitCJK(splitResultList[0])
    ...
    private fracParseCJK(extResult: ExtractResult): ParseResult {
        ...
        const resultText = extResult.text;
        const splitResult = RegExpUtility.split(this.config.fracSplitRegex, resultText);
        let intPart = '';
        let demoPart = '';
        let numPart = '';
    ...
    private douParseCJK(extResult: ExtractResult): ParseResult {
        ...
        if (RegExpUtility.isMatch(this.config.doubleAndRoundRegex, resultText)) {
            resultText = this.replaceUnit(resultText);
            const power = this.config.roundNumberMapChar.get(resultText.charAt(resultText.length - 1))!;
            result.value = this.getDigitValueCJK(resultText.substr(0, resultText.length - 1), power);
        }
        else {
            resultText = this.replaceUnit(resultText);
            const splitResult = RegExpUtility.split(this.config.pointRegex, resultText);
    ...
    private ordParseCJK(extResult: ExtractResult): ParseResult {
        ...
        const resultText = extResult.text.substr(1);
    ...
    private getIntValueCJK(value: string): number {
        ...
        for (let index = 0; index < resultStr.length; index++) {
            const currentChar = resultStr.charAt(index);
            if (this.config.roundNumberMapChar.has(currentChar)) {
                const roundRecent = this.config.roundNumberMapChar.get(currentChar)!;
                ...
                if (index !== resultStr.length - 1) {
                    const is_not_round_next = this.config.tenChars.some(o => o === resultStr.charAt(index + 1)) || !this.config.roundNumberMapChar.has(resultStr.charAt(index + 1));
                    if (currentChar === this.config.zeroChar && is_not_round_next) {
                        beforeValue = 1;
                        roundDefault = 1;
                    }
                    else {
                        const currentDigit = this.config.zeroToNineMap.get(currentChar)!;
