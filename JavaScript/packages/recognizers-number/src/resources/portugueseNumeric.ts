...
export namespace PortugueseNumeric {
    ...
    export const RelativeOrdinalRegex = `(?<relativeOrdinal>pr[oó]xim[ao]s?|[uú]ltim[ao]\\s+mas\\s+um|anterior\\s+ao\\s+últim[ao]|(pen)?[uú]ltim[ao]s?|antepen[uú]ltim[ao]s?|seguintes?|anterior(es)?|atua(l|is))`;
    ...
    export const AllOrdinalRegex = `(?:${AllOrdinalNumberRegex}|${RelativeOrdinalRegex})`;
    ...
    export const FractionNounRegex = `(?<=\\b)(${AllIntRegex}\\s+((e|com)\\s+)?)?((${AllIntRegex})(\\s+((e|com)\\s)?)(((${AllOrdinalNumberRegex})s?|(${SpecialFractionInteger})|(${SuffixRoundOrdinalRegex})s?)|mei[oa]?|ter[çc]o?)|(meio|um\\s+quarto\\s+de)\\s+${RoundNumberIntegerRegex})(?=\\b)`;
    export const FractionNounWithArticleRegex = `(?<=\\b)((${AllIntRegex}|${RoundNumberIntegerRegexWithLocks})\\s+(e\\s+)?)?((um|um[as])(\\s+)((${AllOrdinalNumberRegex})|(${SuffixRoundOrdinalRegex})|(e\\s+)?mei[oa]?)|mei[oa]?)(?=\\b)`;
    ...
    export const AllFloatRegex = `(?<!(entre\\s+(menos\\+)?))${AllIntRegex}(\\s+(v[íi]rgula|e|ponto))${AllPointRegex}`;
    ...
    export const TillRegex = `(\\b[aà]\\b|at[eé]h?|--|-|—|——|~|–)`;
    export const MoreRegex = `(mais\\s+(alt[oa]s?|grandes?)\\s+que|(mais|maior(es)?|superior(es)?|acima)\\b((\\s+(que|de|a))|(?=\\s+ou\\b))|(?<!<|=)>)`;
    export const LessRegex = `(mais\\s+baix[oa]\\s+que|(meno(s|r(es)?)|inferior(es)?|abaixo)(\\s+(que|de|a)|(?=\\s+ou\\b))|(?<!>|=)<)`;
    export const EqualRegex = `((igua(l|is)|equivalente(s)?|equivale(ndo)?)(\\s+(ao?|que|d[eao]))?|(?<!<|>)=)`;
    export const MoreOrEqualPrefix = `((n[ãa]o\\s+${LessRegex})|((pelo|ao)\\s+menos|(como(\\s+o)?|no)\\s+m[íi]nimo))`;
    export const MoreOrEqual = `((${MoreRegex}\\s+(ou)?\\s+${EqualRegex})|(${EqualRegex}\\s+(ou|e)\\s+${MoreRegex})|${MoreOrEqualPrefix}(\\s+(ou)\\s+${EqualRegex})?|(${EqualRegex}\\s+(ou)\\s+)?${MoreOrEqualPrefix}|>\\s*=)`;
    export const MoreOrEqualSuffix = `((\\b(e|ou)\\b\\s+(mais|maior(es)?|superior(es)?)((?!\\s+(alt[oa]|baix[oa]|que|d[eao]|ao?))|(\\s+(que|d[eao]|ao?)(?!(\\s*\\d+)))))|(como(\\s+o)?|no)\\s+m[íi]nimo|(pelo|ao)\\s+menos)\\b`;
    export const LessOrEqualPrefix = `((n[ãa]o\\s+${MoreRegex})|((como(\\s+o)?|no)\\s+m[aá]ximo))`;
    ...
    export const LessOrEqualSuffix = `((\\b(e|ou)\\b\\s+(meno(s|r(es)?|inferior(es)?))((?!\\s+(alt[oa]|baix[oa]|que|d[eao]|ao?))|(\\s+(que|d[eao]|ao?)(?!(\\s*\\d+)))))|(como(\\s+o)?|no)\\s+m[áa]ximo)\\b`;
    export const NumberSplitMark = `(?![,.](?!\\d+))(?!\\s*\\b(((e)\\s+)?(${LessRegex}|${MoreRegex}|${EqualRegex}|n[ãa]o|d[eao])|mas|[ao])\\b)`;
    export const MoreRegexNoNumberSucceed = `(\\b(mais|maior(es)?|superior(es)?)((?!\\s+(que|d[eao]|ao?))|\\s+((que|d[eao])(?!(\\s*\\d+))))|((por\\s+|a)cima)(?!(\\s*\\d+)))\\b`;
    export const LessRegexNoNumberSucceed = `(\\b(meno(s|r(es)?)|inferior(es)?)((?!\\s+(que|d[eao]|ao?))|\\s+((que|d[eao]|ao?)(?!(\\s*\\d+))))|((por\\s+|a)baixo)(?!(\\s*\\d+)))\\b`;
    export const EqualRegexNoNumberSucceed = `(\\b(igua(l|is)|equivalentes?|equivale(ndo)?)((?!\\s+(ao?|que|d[eao]))|(\\s+(ao?|que|d[eao])(?!(\\s*\\d+)))))\\b`;
    export const OneNumberRangeMoreRegex1 = `(${MoreOrEqual}|${MoreRegex})\\s*(([ao]s?)\\s+)?(?<number1>(${NumberSplitMark}.)+)`;
    export const OneNumberRangeMoreRegex1LB = `(?<!n[ãa]o\\s+)${OneNumberRangeMoreRegex1}`;
    ...
    export const OneNumberRangeLessRegex1 = `(${LessOrEqual}|${LessRegex})\\s*([ao]s?\\s+)?(?<number2>(${NumberSplitMark}.)+)`;
    export const OneNumberRangeLessRegex1LB = `(?<!n[ãa]o\\s+)${OneNumberRangeLessRegex1}`;
    ...
    export const OneNumberRangeEqualRegex = `${EqualRegex}\\s*([ao]s?\\s+)?(?<number1>(${NumberSplitMark}.)+)`;
    export const TwoNumberRangeRegex1 = `\\bentre\\s*([ao]s?\\s+)?(?<number1>(${NumberSplitMark}.)+)\\s*e\\s*([ao]s?\\s+)?(?<number2>(${NumberSplitMark}.)+)`;
    export const TwoNumberRangeRegex2 = `(${OneNumberRangeMoreRegex1}|${OneNumberRangeMoreRegex2})\\s*(\\be\\b|mas|,)\\s*(${OneNumberRangeLessRegex1}|${OneNumberRangeLessRegex2})`;
    export const TwoNumberRangeRegex3 = `(${OneNumberRangeLessRegex1}|${OneNumberRangeLessRegex2})\\s*(\\be\\b|mas|,)\\s*(${OneNumberRangeMoreRegex1}|${OneNumberRangeMoreRegex2})`;
    export const TwoNumberRangeRegex4 = `(\\bde(sde)?\\s+)?(\\b[ao]s?\\s+)?\\b(?!\\s+)(?<number1>(${NumberSplitMark}(?!\\b(entre|de(sde)?|es)\\b).)+)\\b\\s*${TillRegex}\\s*([ao]s?\\s+)?\\b(?!\\s+)(?<number2>(${NumberSplitMark}.)+)\\b`;
    export const AmbiguousFractionConnectorsRegex = `(\\b(em|de)\\b)`;
    ...
    export const DigitalNumberRegex = `((?<=\\b)(mil(hares)?|ce(m|ntenas?)|[bmt]ilh([aã]o|[oõ]es)|dezenas?)(?=\\b))|((?<=(\\d|\\b))${BaseNumbers.MultiplierLookupRegex}(?=\\b))`;
    ...
    export const RelativeReferenceOffsetMap: ReadonlyMap<string, string> = new Map<string, string>([["proxima", "1"],["proximo", "1"],["próxima", "1"],["próximo", "1"],["proximas", "1"],["proximos", "1"],["próximas", "1"],["próximos", "1"],["ultima", "0"],["ultimo", "0"],["última", "0"],["último", "0"],["ultimas", "0"],["ultimos", "0"],["últimas", "0"],["últimos", "0"],["penultima", "-1"],["penultimo", "-1"],["penúltima", "-1"],["penúltimo", "-1"],["penultimas", "-1"],["penultimos", "-1"],["penúltimas", "-1"],["penúltimos", "-1"],["ultima mas um", "-1"],["ultimo mas um", "-1"],["última mas um", "-1"],["último mas um", "-1"],["anterior ao último", "-1"],["anterior ao última", "-1"],["antepenultima", "-2"],["antepenultimo", "-2"],["antepenúltima", "-2"],["antepenúltimo", "-2"],["antepenultimas", "-2"],["antepenultimos", "-2"],["antepenúltimas", "-2"],["antepenúltimos", "-2"],["seguinte", "1"],["seguintes", "1"],["anterior", "-1"],["anteriores", "-1"],["atual", "0"],["atuais", "0"]]);
    export const RelativeReferenceRelativeToMap: ReadonlyMap<string, string> = new Map<string, string>([["proxima", "current"],["proximo", "current"],["próxima", "current"],["próximo", "current"],["proximas", "current"],["proximos", "current"],["próximas", "current"],["próximos", "current"],["ultima", "end"],["ultimo", "end"],["última", "end"],["último", "end"],["ultimas", "end"],["ultimos", "end"],["últimas", "end"],["últimos", "end"],["penultima", "end"],["penultimo", "end"],["penúltima", "end"],["penúltimo", "end"],["penultimas", "end"],["penultimos", "end"],["penúltimas", "end"],["penúltimos", "end"],["ultima mas um", "end"],["ultimo mas um", "end"],["última mas um", "end"],["último mas um", "end"],["anterior ao último", "end"],["anterior ao última", "end"],["antepenultima", "end"],["antepenultimo", "end"],["antepenúltima", "end"],["antepenúltimo", "end"],["antepenultimas", "end"],["antepenultimos", "end"],["antepenúltimas", "end"],["antepenúltimos", "end"],["seguinte", "current"],["seguintes", "current"],["anterior", "current"],["anteriores", "current"],["atual", "current"],["atuais", "current"]]);
}
