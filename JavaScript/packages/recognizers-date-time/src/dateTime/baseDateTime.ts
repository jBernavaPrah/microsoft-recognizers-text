...
export class BaseDateTimeExtractor implements IDateTimeExtractor {
    ...
    protected mergeDateAndTime(source: string, refDate: Date): Token[] {
        ...
        while (i < ers.length - 1) {
            ...
            if ((ers[i].type === Constants.SYS_DATETIME_DATE && ers[j].type === Constants.SYS_DATETIME_TIME) ||
                ...
                if (this.config.isConnectorToken(middleStr)) {
                    const begin = ers[i].start;
                    const end = ers[j].start + ers[j].length;
        ...
        tokens.forEach((token, index) => {
            const afterStr = source.substr(token.end);
            const match = RegExpUtility.getMatches(this.config.suffixRegex, afterStr);
    ...
    private timeOfTodayBefore(source: string, refDate: Date): Token[] {
        const tokens: Token[] = new Array<Token>();
        const ers = this.config.timePointExtractor.extract(source, refDate);
        ers.forEach(er => {
            let beforeStr = source.substr(0, er.start);
            const innerMatches = RegExpUtility.getMatches(this.config.nightRegex, er.text);
            ...
            const matches = RegExpUtility.getMatches(this.config.timeOfTodayBeforeRegex, beforeStr);
            if (matches && matches.length > 0) {
                const begin = matches[0].index;
    ...
    private timeOfTodayAfter(source: string, refDate: Date): Token[] {
        ...
        ers.forEach(er => {
            ...
            const matches = RegExpUtility.getMatches(this.config.timeOfTodayAfterRegex, afterStr);
            if (matches && matches.length > 0) {
                const begin = er.start;
                const end = er.start + er.length + matches[0].length;
                tokens.push(new Token(begin, end));
            }
        });
    ...
    private specialTimeOfDate(source: string, refDate: Date): Token[] {
        ...
        ers.forEach(er => {
            ...
            const beforeMatches = RegExpUtility.getMatches(this.config.specificEndOfRegex, beforeStr);
            if (beforeMatches && beforeMatches.length > 0) {
                tokens.push(new Token(beforeMatches[0].index, er.start + er.length));
            }
            else {
                const afterStr = source.substr(er.start + er.length);
                const afterMatches = RegExpUtility.getMatches(this.config.specificEndOfRegex, afterStr);
    ...
    protected durationWithBeforeAndAfter(source: string, refDate: Date): Token[] {
        ...
        this.config.durationExtractor.extract(source, refDate).forEach(er => {
            const matches = RegExpUtility.getMatches(this.config.unitRegex, er.text);
            if (matches && matches.length > 0) {
...
export class BaseDateTimeParser implements IDateTimeParser {
    ...
    protected parseBasicRegex(text: string, referenceTime: Date): DateTimeResolutionResult {
        ...
        // handle "now"
        const matches = RegExpUtility.getMatches(this.config.nowRegex, trimmedText);
        if (matches.length && matches[0].index === 0 && matches[0].length === trimmedText.length) {
            const getMatchedNowTimex = this.config.getMatchedNowTimex(trimmedText);
    ...
    protected mergeDateAndTime(text: string, referenceTime: Date): DateTimeResolutionResult {
        ...
        const pr1 = this.config.dateParser.parse(er1[0], new Date(referenceTime.toDateString()))!;
        const pr2 = this.config.timeParser.parse(er2[correctTimeIdx], referenceTime)!;
        if (pr1.value === null || pr2.value === null) {
            return ret;
        }

        const futureDate = pr1.value.futureValue;
        const pastDate = pr1.value.pastValue;
        const time = pr2.value.futureValue;
    ...
    protected parseTimeOfToday(text: string, referenceTime: Date): DateTimeResolutionResult {
        ...
        if (wholeMatches.length && wholeMatches[0].length === trimmedText.length) {
            ...
            const pr = this.config.timeParser.parse(ers[0], referenceTime)!;
            if (pr.value === null) {
                return ret;
            }

            const time = pr.value.futureValue;
        ...
        const matches = RegExpUtility.getMatches(this.config.specificTimeOfDayRegex, trimmedText);

        if (matches.length) {
            const matchStr = matches[0].value.toLowerCase();

            // handle "last", "next"
            const swift = this.config.getSwiftDay(matchStr);
    ...
    private parseSpecialTimeOfDate(text: string, refDateTime: Date): DateTimeResolutionResult {
        ...
        const ers = this.config.dateExtractor.extract(text, refDateTime);
        if (ers.length !== 1) {
            return ret;
        }
        const beforeStr = text.substring(0, ers[0].start || 0);
        if (RegExpUtility.getMatches(this.config.specificEndOfRegex, beforeStr).length) {
            const pr = this.config.dateParser.parse(ers[0], refDateTime)!;
            const futureDate = new Date(pr.value.futureValue);
            const pastDate = new Date(pr.value.pastValue);
    ...
    private parseUnspecificTimeOfDate(text: string, refDateTime: Date): DateTimeResolutionResult {
        let ret = new DateTimeResolutionResult();
        const eod = RegExpUtility.getMatches(this.config.unspecificEndOfRegex, text);
        if (eod.length) {
            const futureDate = new Date(refDateTime);
            const pastDate = new Date(refDateTime);
