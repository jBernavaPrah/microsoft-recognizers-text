...
export class BaseSetExtractor implements IDateTimeExtractor {
    ...
    protected matchEachDuration(source: string, refDate: Date): Token[] {
        ...
        this.config.durationExtractor.extract(source, refDate).forEach(er => {
            ...
            const matches = RegExpUtility.getMatches(this.config.eachPrefixRegex, beforeStr);
    ...
    protected timeEveryday(source: string, refDate: Date): Token[] {
        ...
        this.config.timeExtractor.extract(source, refDate).forEach(er => {
            ...
            if (StringUtility.isNullOrWhitespace(afterStr) && this.config.beforeEachDayRegex) {
                const beforeStr = source.substr(0, er.start);
                const beforeMatches = RegExpUtility.getMatches(this.config.beforeEachDayRegex, beforeStr);
                if (beforeMatches && beforeMatches.length > 0) {
                    ret.push(new Token(beforeMatches[0].index, er.start + er.length));
                }
            }
            else {
                const afterMatches = RegExpUtility.getMatches(this.config.eachDayRegex, afterStr);
    ...
    private matchEach(extractor: IDateTimeExtractor, source: string, refDate: Date): Token[] {
        ...
        RegExpUtility.getMatches(this.config.setEachRegex, source).forEach(match => {
            const trimmedSource = source.substr(0, match.index) + source.substr(match.index + match.length);
        ...
        RegExpUtility.getMatches(this.config.setWeekDayRegex, source).forEach(match => {
            const trimmedSource = source.substr(0, match.index) + match.groups('weekday').value + source.substr(match.index + match.length);
            extractor.extract(trimmedSource, refDate).forEach(er => {
...
export class BaseSetParser implements IDateTimeParser {
    ...
    protected parseEachDuration(text: string, refDate: Date): DateTimeResolutionResult {
        ...
        const ers = this.config.durationExtractor.extract(text, refDate);
        ...
        const matches = RegExpUtility.getMatches(this.config.eachPrefixRegex, beforeStr);
        if (matches.length) {
            const pr = this.config.durationParser.parse(ers[0], new Date())!;
    ...
    protected parseEachUnit(text: string): DateTimeResolutionResult {
        ...
        if (matches.length) {
            const getMatchedDailyTimex = this.config.getMatchedDailyTimex(text);
        ...
        if (matches.length && matches[0].length === text.length) {
            const sourceUnit = matches[0].groups("unit").value;
            if (sourceUnit && this.config.unitMap.has(sourceUnit)) {
                const getMatchedUnitTimex = this.config.getMatchedUnitTimex(sourceUnit);
    ...
    protected parserTimeEveryday(text: string, refDate: Date): DateTimeResolutionResult {
        const ret = new DateTimeResolutionResult();
        const ers = this.config.timeExtractor.extract(text, refDate);
        if (ers.length !== 1) {
            return ret;
        }

        const afterStr = text.replace(ers[0].text, "");
        const matches = RegExpUtility.getMatches(this.config.eachDayRegex, afterStr);
        if (matches.length) {
            const pr = this.config.timeParser.parse(ers[0], new Date());
            ret.timex = pr.timexStr;
            ret.futureValue = ret.pastValue = "Set: " + ret.timex;
            ret.success = true;
            return ret;
    ...
    protected parseEach(extractor: IDateTimeExtractor, parser: IDateTimeParser, text: string, refDate: Date): DateTimeResolutionResult {
        ...
        if (match) {
            const trimmedText = text.substr(0, match.index) + text.substr(match.index + match.length);
        ...
        if (match) {
            const trimmedText = text.substr(0, match.index) + match.groups('weekday').value + text.substr(match.index + match.length);
            er = extractor.extract(trimmedText, refDate);
            if (er.length === 1 && er[0].length === trimmedText.length) {
                success = true;
            }
