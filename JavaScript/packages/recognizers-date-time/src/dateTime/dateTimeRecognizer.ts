...
export function recognizeDateTime(query: string, culture: string, options: DateTimeOptions = DateTimeOptions.None,
    ...
    const recognizer = new DateTimeRecognizer(culture, options);
    const model = recognizer.getDateTimeModel(culture, fallbackToDefaultCulture);
    return model.parse(query, referenceDate);
}

export default class DateTimeRecognizer extends Recognizer<DateTimeOptions> {
    constructor(culture: string, options: DateTimeOptions | number = DateTimeOptions.None, lazyInitialization: boolean = false) {
    ...
    getDateTimeModel(culture: string | null = null, fallbackToDefaultCulture: boolean = true): IDateTimeModel {
