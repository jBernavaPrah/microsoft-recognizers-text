...
import { IDatePeriodParserConfiguration } from './baseDatePeriod';

export class Token {
    constructor(start: number, end: number, metaData: MetaData | null = null) {
        this.start = start;
        this.end = end;
        ...
        tokens.forEach(token => {
            if (token) {
                ...
                for (let index = 0; index < mergedTokens.length && bAdd; index++) {
                    const mergedToken = mergedTokens[index];
...
export class AgoLaterUtil {
    static extractorDurationWithBeforeAndAfter(source: string, er: ExtractResult, ret: Token[], config: IDateTimeUtilityConfiguration): Token[] {
        const pos = er.start + er.length;
        if (pos <= source.length) {
            const afterString = source.substring(pos);
            const beforeString = source.substring(0, er.start);
            const index = -1;
    ...
    static parseDurationWithAgoAndLater(source: string, referenceDate: Date, durationExtractor: IDateTimeExtractor, durationParser: IDateTimeParser, unitMap: ReadonlyMap<string, string>, unitRegex: RegExp, utilityConfiguration: IDateTimeUtilityConfiguration, mode: AgoLaterMode): DateTimeResolutionResult {
        ...
        const duration = durationExtractor.extract(source, referenceDate).pop();
        if (!duration) {
            return result;
        }
        const pr = durationParser.parse(duration, referenceDate);
        if (!pr) {
            return result;
        }
        const match = RegExpUtility.getMatches(unitRegex, source).pop();
        if (!match) {
            return result;
        }
        const afterStr = source.substr(duration.start + duration.length);
        const beforeStr = source.substr(0, duration.start);
        const srcUnit = match.groups('unit').value;
        const durationResult: DateTimeResolutionResult = pr.value;
        const numStr = durationResult.timex.substr(0, durationResult.timex.length - 1)
            .replace('P', '')
            .replace('T', '');
    ...
    static getAgoLaterResult(durationParseResult: DateTimeParseResult, num: number, unitMap: ReadonlyMap<string, string>, srcUnit: string, afterStr: string, beforeStr: string, referenceDate: Date, utilityConfiguration: IDateTimeUtilityConfiguration, mode: AgoLaterMode) {
        ...
        const unitStr = unitMap.get(srcUnit);
        if (!unitStr) {
            return result;
        }
        const numStr = num.toString();
        const containsAgo = MatchingUtil.containsAgoLaterIndex(afterStr, utilityConfiguration.agoRegex);
        const containsLaterOrIn = MatchingUtil.containsAgoLaterIndex(afterStr, utilityConfiguration.laterRegex) || MatchingUtil.containsInIndex(beforeStr, utilityConfiguration.inConnectorRegex);
        if (containsAgo) {
...
export class MatchingUtil {
    ...
    public static getAgoLaterIndex(source: string, regex: RegExp): MatchedIndex {
        const result: MatchedIndex = { matched: false, index: -1 };
        const referencedMatches = RegExpUtility.getMatches(regex, source.trim().toLowerCase());
    ...
    public static getInIndex(source: string, regex: RegExp): MatchedIndex {
        const result: MatchedIndex = { matched: false, index: -1 };
        const referencedMatch = RegExpUtility.getMatches(regex, source.trim().toLowerCase().split(' ').pop()!);
...
export class DateTimeFormatUtil {
    ...
    public static luisDateShortTime(time: Date, timex: string | null = null): string {
        const hasMin = timex != null ? timex.includes(Constants.TimeTimexConnector) : false;
        const hasSec = timex != null ? timex.split(Constants.TimeTimexConnector).length > 2 : false;
    ...
    public static formatShortTime(time: Date, hasMin: boolean = false, hasSec: boolean = false): string {
        ...
        const min = hasMin || time.getMinutes() > 0 ? time.getMinutes() : -1;
        const sec = hasSec || time.getSeconds() > 0 ? time.getSeconds() : -1;
    ...
    public static allStringToPm(timeStr: string): string {
        const matches = RegExpUtility.getMatches(DateTimeFormatUtil.HourTimexRegex, timeStr);
...
export class DateUtils {
    ...
    static parseChineseDynastyYear(yearStr: string, dynastyYearRegex: RegExp, dynastyYearMap: ReadonlyMap<string, number>, dynastyStartYear: string, integerExtractor: IExtractor, numberParser: IParser): number {
        let year = -1;
        const regionTitleMatch = RegExpUtility.getMatches(dynastyYearRegex, yearStr).pop();
        if (regionTitleMatch && regionTitleMatch.index === 0 && regionTitleMatch.length === yearStr.length) {
            ...
            const dynastyYearStr = regionTitleMatch.groups('dynasty').value;
            const biasYearStr = regionTitleMatch.groups('biasYear').value;
            const basicYear = dynastyYearMap.get(dynastyYearStr)!;
            let biasYear = 1;
            if (biasYearStr != dynastyStartYear) {
                const er = integerExtractor.extract(biasYearStr).pop()!;
    ...
    static next(from: Date, dayOfWeek: DayOfWeek): Date {
        let start: number = from.getDay();
        let target: number = dayOfWeek;
    ...
    static totalHours(from: Date, to: Date): number {
        ...
        // JS: Math.round(4.5) === 5 !!
        const fromEpoch = from.getTime() - (from.getTimezoneOffset() * 60 * 1000);
        const toEpoch = to.getTime() - (to.getTimezoneOffset() * 60 * 1000);
        return Math.round(Math.abs(fromEpoch - toEpoch - 0.00001) / this.oneHour);
    }

    static totalHoursFloor(from: Date, to: Date): number {
        const fromEpoch = from.getTime() - (from.getTimezoneOffset() * this.oneMinute);
        const toEpoch = to.getTime() - (to.getTimezoneOffset() * this.oneMinute);
    ...
    static getWeekNumber(referenceDate: Date): { weekNo: number, year: number } {
        ...
        const target = new Date(referenceDate.valueOf());

        // ISO week date weeks start on monday
        // so correct the day number
        const dayNr = (referenceDate.getDay() + 6) % 7;
        ...
        const weekNo = 1 + Math.ceil((firstThursday - target.valueOf()) / 604800000); // 604800000 = 7 * 24 * 3600 * 1000
        return { weekNo: weekNo, year: thursday.getFullYear() };
    }

    static minValue(): Date {
        const date = new Date(1, 0, 1, 0, 0, 0, 0);
        date.setFullYear(1);
        return date;
    }
    ...
    static dayOfYear(date: Date): number {
        ...
        const diffDays = date.valueOf() - start.valueOf();
    ...
    static getYear(config: IDatePeriodParserConfiguration, startText: string, endText: string, text: string): number {
        let contextYear = -1;
        let isEndDatePureYear = false;
        let isDateRelative = false;

        const yearMatchForEndDate = RegExpUtility.getMatches(config.yearRegex, endText);

        if (yearMatchForEndDate && yearMatchForEndDate.length == 1 && yearMatchForEndDate[0].length == endText.length) {
            isEndDatePureYear = true;
        }

        const relativeMatchForStartDate = RegExpUtility.getMatches(config.relativeRegex, startText);
        const relativeMatchForEndDate = RegExpUtility.getMatches(config.relativeRegex, endText);
        ...
        if (!isEndDatePureYear && !isDateRelative) {
            const matchYear = RegExpUtility.getMatches(config.yearRegex, text);
            for (const match of matchYear) {
                const year = AbstractYearExtractor.getYearFromText(match, config.numberParser);
                if (year != -1) {
                    if (contextYear == -1) {
                        contextYear = year;
                    }
...
export class TimexUtil {
    public static parseTimeOfDay(tod: string): TimeOfDayResolutionResult {
        const result = new TimeOfDayResolutionResult();
        switch (tod) {
    ...
    public static generateDatePeriodTimex(begin: Date, end: Date, timexType: string, timex1: string, timex2: string) {
        const boundaryValid = !DateUtils.isDafaultValue(begin) && !DateUtils.isDafaultValue(end);
        const unitCount = boundaryValid ? TimexUtil.getDatePeriodTimexUnitCount(begin, end, timexType) : 'X';
        const datePeriodTimex = `P${unitCount}${Constants.DatePeriodTimexTypeToTimexSuffix.get(timexType)}`;
...
export class AbstractYearExtractor {
    ...
    public static getYearFromText(match: Match, numberParser: BaseNumberParser): number {
        ...
        const writtenYearStr = match.groups('fullyear').value;
        if (!StringUtility.isNullOrEmpty(yearStr) && !(yearStr == writtenYearStr)) {
            ...
            const firstTwoYearNumStr = match.groups('firsttwoyearnum').value;
            if (!StringUtility.isNullOrEmpty(firstTwoYearNumStr)) {
                const er = new ExtractResult();
                er.text = firstTwoYearNumStr;
                er.start = match.groups('firsttwoyearnum').index;
                er.length = match.groups('firsttwoyearnum').length;

                const firstTwoYearNum = Number.parseInt(numberParser.parse(er)!.value, 10);

                let lastTwoYearNum = 0;
                const lastTwoYearNumStr = match.groups('lasttwoyearnum').value;
...
export class DefinitionLoader {
    ...
    public static LoadAmbiguityFilters(filters: ReadonlyMap<string, string>) {
        const ambiguityFiltersDict: Map<RegExp, RegExp> = new Map();
