...
interface Groups {
    [id: string]: { value: string, index: number, length: number, captures: string[] };
}

export class Match {
    constructor(index: number, length: number, value: string, groups: Groups) {
        this.index = index;
        this.length = length;
        this.value = value;
    ...
    length: number;
    value: string;
    private readonly innerGroups: Groups;
...
export class ConditionalMatch {
    constructor(match: Match | undefined, success: boolean) {
        this.match = match;
...
export class RegExpUtility {
    ...
    static getMatchEnd(regex: RegExp, source: string, trim: boolean): ConditionalMatch {
        const match = this.getMatches(regex, source).pop();
        let strAfter = '';
        if (match) {
        ...
        return new ConditionalMatch(match, match !== undefined && StringUtility.isNullOrEmpty(strAfter));
    }
    ...
    static getMatchesSimple(regex: RegExp, source: string): Match[] {
        ...
        const normalized = StringUtility.removeDiacriticsFromWordBoundaries(source);
        ...
        XRegExp.forEach(normalized, regex, match => {
            ...
            Object.keys(match).forEach(key => {
                ...
                const groupKey = key.substring(0, key.lastIndexOf('__'));
                ...
                if (match[key]) {
                    const index = match.index! + match[0].indexOf(match[key]);
                    const length = match[key].length;
                    const value = source.substring(index, length);
            ...
            value = source.substring(index, length);
    ...
    static getSafeRegExp(source: string, flags?: string): RegExp {
        return XRegExp(this.sanitizeGroups(source), flags || 'gis');
    }
    ...
    static getFirstMatchIndex(regex: RegExp, source: string): {
        ...
        const matches = RegExpUtility.getMatches(regex, source);
        if (matches.length) {
            return {
                matched: true,
    ...
    private static sanitizeGroups(source: string): string {
        let index = 0;
        return XRegExp.replace(source, this.matchGroup, (match, name) => match.replace(`${name}`, `${name}__${index++}`));
    }
...
export class StringUtility {
    static isNullOrWhitespace(input: string | null | undefined): boolean {
    ...
    static removeDiacritics(c: string): string {
        return this.diacriticsRemovalMap[c] || c;
    }

    private static readonly diacriticsRemovalMap: { [key: string]: string } = {
        'Ⓐ': 'A',
        'Ａ': 'A',
        'À': 'A',
        'Á': 'A',
